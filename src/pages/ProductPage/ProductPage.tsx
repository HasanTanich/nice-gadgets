...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    if (sortTypeOptions.find((item) => item.value === sortType) === undefined) {
      ...
      itemsOnPageOptions.find((item) => item.value === itemsPerPage) ===
      undefined
    ) {
  ...
  const { isLoading, isError, data, productDetailsFetchUrl } = GetProductData(
    product,
    productType,
    productId,
    searchQuery
  ...
  return (
    <>
      <div className="breadcrumbs">
        ...
        <p className={`breadcrumbs-product ${productId ? "currentPage" : ""}`}>
          {product}
        </p>
      ...
      <div className="container">
        {productId && productDetailsFetchUrl && (
          <ProductDetailsPage url={productDetailsFetchUrl} />
        )}
